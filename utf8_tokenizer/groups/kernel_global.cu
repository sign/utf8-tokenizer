/*
 * CUDA kernel for UTF-8 byte grouping using global memory.
 * Supports seq_len up to 64K (slower than shared memory version).
 */
...
#define MAX_SEQ_LEN_GLOBAL 65536

__global__ void group_utf8_bytes_kernel_global(
    const int64_t* __restrict__ input,
    int64_t* __restrict__ output,
    int* __restrict__ num_groups_out,
    int* __restrict__ workspace,  // (batch, 4 * seq_len)
    ...
    // Global memory work arrays
    int* is_leading = workspace + batch_idx * 4 * seq_len;
    int* group_idx = is_leading + seq_len;
    int* group_start_pos = group_idx + seq_len;
    int* group_lengths = group_start_pos + seq_len;
...
torch::Tensor group_utf8_bytes_cuda_global(
    torch::Tensor input,
    int padding_value
) {
    ...
    TORCH_CHECK(seq_len <= MAX_SEQ_LEN_GLOBAL,
                "Sequence length ", seq_len, " exceeds maximum: ", MAX_SEQ_LEN_GLOBAL);
    ...
    auto workspace = torch::zeros({batch_size * 4 * seq_len}, input.options().dtype(torch::kInt32));

    const int threads_per_block = 256;

    group_utf8_bytes_kernel_global<<<batch_size, threads_per_block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("group_utf8_bytes_cuda", &group_utf8_bytes_cuda_global,
          "UTF-8 grouping using global memory (seq_len <= 64K)",
          py::arg("input"),
