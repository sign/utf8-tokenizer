...
class TestLogitsProcessing:
    ...
    def test_call_batch_processing(self, processor):
        ...
        # First sequence (ASCII complete) - no masking, scores unchanged
        for i in range(256):
            assert processed_scores[0, i] == 0.0
    ...
    def test_empty_sequence_no_masking(self, processor):
        """Test that empty sequence applies no masking."""
...
class TestRealWorldSequences:
    ...
    def test_mixed_sequence(self, processor):
        ...
        # After complete "Hello©", no masking (complete character)
...
class TestGenerationWithUTF8LMTiny:
    """Test generation with the sign/utf8-lm-tiny model."""
    ...
    def _prepare_inputs(self, tokenizer, prompt):
        inputs = tokenizer([prompt], return_tensors="pt", padding=True, add_special_tokens=True)
        inputs["input_ids"] = inputs["input_ids"].to(torch.long)[:, :-1]
        inputs["attention_mask"] = inputs["attention_mask"][:, :-1]
        return inputs

    def test_generation_identical_with_and_without_processor(self, model_and_tokenizer):
        ...
        with torch.no_grad():
            out_without = model.generate(**inputs, max_new_tokens=10, do_sample=False)
            out_with = model.generate(**inputs, max_new_tokens=10, do_sample=False, logits_processor=[processor])

        assert out_without.tolist() == out_with.tolist()
